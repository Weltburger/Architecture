...
;---------------------------------
s db 0
X db 12 DUP(?) ; Резервируем память под массив
svX db 10,13,'Enter an array element X!',10,13,'$'
svXXX db 10,13,'Array X:',10,13,'$'
ssum db 'The sum of elements with even numbers of columns in odd rows= $'
...
call vivod
pop bx cx
add si,1 ; переход к след. элементу строки
loop @next_elem
...
@next_elem1:
push cx
mov al,X[bx][si] ; берем очередной элемент
;test k1,00000001b ; проверяем строку на четность
;jnz @dalhe
; нечетная строка
test k,00000001b ; проверяем столбец на четность
jz @dalhe
...
@mq2: test al,00000001b ; проверяем нулевой бит байта
je @mq3 		; если он=0, то продолжаем цикл
add s,1 	; наращиваем счетчик единиц
@mq3: shr al,1 ; сдвигаем al вправо на один бит
loop @mq2
pop ax
...
cmp s,7
je @dalhe
; нечетная строка и четный столбец – наращиваем сумму
add sum,ax
; переход к следующему элементу строки
@dalhe: add si,1 ; адрес след. элемента
